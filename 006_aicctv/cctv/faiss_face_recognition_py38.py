...
# 수정이력 : Python 3.11 → 3.8 호환 변환
#            - typing 모듈 사용 (List, Dict, Optional, Tuple)
#            - Union 타입 | 연산자 → Optional 으로 변경
##########################################################

from __future__ import annotations  # 타입 힌트 지연 평가 (3.8 호환)
...
class FaceRecognitionFAISS:
    ...
    def __init__(
        ...
        # 런타임에 로드되는 객체
        # [3.8 변경] faiss.Index | None  →  Optional[faiss.Index]
        self.face_index: Optional[faiss.Index] = None
    ...
    def _expand_bbox(
        ...
        # [3.8 변경] 반환 타입 명시를 위해 Tuple[int, int, int, int] 사용
        """얼굴 bounding box를 face_margin만큼 확장한다."""
        m = self.face_margin
        return top - m, right + m, bottom + m, left - m

    @staticmethod
    def _most_frequent(data: list) -> Tuple[object, int]:
        # [3.8 변경] 반환 타입 명시를 위해 Tuple 사용
    ...
    def _get_index_path(self) -> str:
        ...
        return os.path.join(self.train_dir, "face_{}.bin".format(today))

    def _get_labels_path(self, date_str: Optional[str] = None) -> str:
        if date_str is None:
            date_str = datetime.today().strftime("%Y%m%d")
        return os.path.join(self.train_dir, "labels_{}.npy".format(date_str))
    ...
    def create_dataset(self) -> Dict[str, str]:
        # [3.8 변경] dict[str, str]  →  Dict[str, str]
        """
        ...
        labels_txt = os.path.join("./dataset/train", "labels_{}.txt".format(today))
        with open(labels_txt, "w", encoding="utf-8") as f:
            for file in file_list:
                ...
                print("{},{}".format(eng_name, kor_name), file=f)
    ...
    def check_faces(self) -> None:
        ...
        if test_images:
            ...
            for path_img in test_images:
                ...
                face_count = len(img_face)
                status = (
                    "✔ 정상" if face_count == 1 else "⚠ 얼굴 {}개".format(face_count)
                )
                print(
    ...
    def train(self, test_img_path: Optional[str] = None) -> faiss.Index:
        # [3.8 변경] str | None  →  Optional[str]
        """
    ...
    def load_model(self, index_path: Optional[str] = None) -> None:
        # [3.8 변경] str | None  →  Optional[str]
        """
        ...
        # .bin 파일명에서 날짜(YYYYMMDD)를 추출해 라벨 파일도 같은 날짜로 맞춘다.
        import re as _re
        _m = _re.search(r"(\d{8})", os.path.basename(index_path))
        _date = _m.group(1) if _m else None
        labels_path = self._get_labels_path(_date)
    ...
    def predict_numpy(self, bgr_frame: np.ndarray) -> str:
        """
        OpenCV BGR numpy 배열로부터 얼굴을 인식하여 이름을 반환한다.

        Parameters
        ----------
        bgr_frame : cv2.VideoCapture로 읽은 BGR numpy 배열
        ...
        # OpenCV BGR → RGB (face_recognition은 RGB 기대)
        rgb_frame = cv2.cvtColor(bgr_frame, cv2.COLOR_BGR2RGB)

        test_face = face_recognition.face_locations(rgb_frame)

        if len(test_face) != 1:
            print(f"[predict_numpy] 얼굴 검출 실패 (검출 수: {len(test_face)})")
            return "unknown"

        top, right, bottom, left = test_face[0]
        print(
            f"[predict_numpy] 얼굴 좌표: top={top}, right={right}, bottom={bottom}, left={left}"
        )

        top, right, bottom, left = self._expand_bbox(top, right, bottom, left)
        face_cut = rgb_frame[top:bottom, left:right]
        ...
        print(f"[predict_numpy] top-{self.top_k} 라벨: {label}")
        print(f"[predict_numpy] top-{self.top_k} 거리: {distance}")

        name, vote_count = self._most_frequent(label)

        if vote_count < self.min_vote:
            print(f"[predict_numpy] 투표 수 부족 ({vote_count} < {self.min_vote}) → unknown")
            return "unknown"

        print(f"[predict_numpy] 예측 결과: {name} (투표 수: {vote_count})")
        return name

    def predict_numpy_detail(self, bgr_frame: np.ndarray) -> dict:
        """predict_numpy와 동일하나 신뢰도·거리 정보를 dict로 함께 반환한다."""
        _empty = {'name': 'unknown', 'confidence': 0.0, 'distance': 999.0, 'vote': 0}
        ...
        dist_val   = float(distance[0][0])
        confidence = round((vote_count / self.top_k) * 100.0, 1)

        if vote_count < self.min_vote:
            return {'name': 'unknown', 'confidence': confidence, 'distance': dist_val, 'vote': vote_count}

        return {'name': name, 'confidence': confidence, 'distance': dist_val, 'vote': vote_count}

    # ------------------------------------------------------------------ #
    #  편의 메서드 : 전체 파이프라인 일괄 실행                               #
    # ------------------------------------------------------------------ #

    def run_pipeline(self, test_img_path: Optional[str] = None) -> None:
        # [3.8 변경] str | None  →  Optional[str]
        """
...
#     # ── 예측만 (모델이 이미 학습된 경우) ────────────────────────────
#     fr.load_model("./dataset/train/face_20260518.bin")  # 날짜 맞게 수정
