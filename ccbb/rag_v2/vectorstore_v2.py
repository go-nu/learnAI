"""
vectorstore_v2.py — 임베딩·Chroma VectorStore·검색 Mixin
BGE-M3 임베딩 생성, Chroma DB 빌드/로드, 유사도 검색 메서드 모음입니다.
텍스트·표만 임베딩 대상이며, 이미지 Document는 생성하지 않습니다.

v1과의 차이
-----------
- create_vectorstore() : 이미지 Document 통계 제거
- build_rag_components() : n_image 재빌드 조건 제거 (이미지 Document 없음)
- similarity_search() 등 : image_refs 메타데이터 출력 추가
"""
