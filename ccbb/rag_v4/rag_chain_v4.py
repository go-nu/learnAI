"""
rag_chain_v4.py — LLM·프롬프트·RAG 체인 Mixin
"""
...
def _rerank_by_case_title(
    ...
    """
    벡터 검색 결과를 case_title 기반으로 재순위화합니다.

    - Chroma L2 점수는 낮을수록 유사 → boost는 점수에서 차감
    - 완전일치: boost 전량 차감 (score - boost)
    - 토큰 Jaccard 일치율에 비례한 boost 차감
    - case_title 없는 Document는 점수 변경 없음
    """
